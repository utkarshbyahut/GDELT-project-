#!/usr/bin/env python3
"""
Fetch FEMA Disaster Declarations and build an enriched county–event NDJSON spine.

Enhancements:
- Add incidentBeginDate, incidentEndDate, declarationType, declarationTitle
- Compute per-record time windows (window_start/window_end) around the incident dates
- Emit a human-readable label per event
- Track skip reasons in the log
"""
...
# default window padding around incident dates
PRE_DAYS_DEFAULT  = 7
POST_DAYS_DEFAULT = 14
...
def norm_date(date_str: str) -> Optional[str]:
    if not date_str:
        return None
    # FEMA timestamps are ISO-like; we keep YYYY-MM-DD
    return date_str[:10]
...
def parse_date(date_str: Optional[str]) -> Optional[datetime]:
    ...
    ds = date_str.strip()
    try:
        if len(ds) >= 19 and ("T" in ds):
            # Full ISO, possibly with Z / offset
            # strip subsecond and parse relatively leniently
            ds = ds.split(".")[0]
            # ensure Z as UTC if present
            if ds.endswith("Z"):
                return datetime.fromisoformat(ds.replace("Z", "+00:00"))
            # if no offset, treat as UTC
            if "+" not in ds and "-" in ds[10:]:
...
def write_log_row(log_path: Path, row):
    ...
    with log_path.open("a", newline="", encoding="utf-8") as f:
        ...
        if need_header:
            w.writerow([
                "date", "state", "county_fips", "county",
                "incidentType", "disaster", "disasterNumber",
                "window_start", "window_end", "skip_reason"
            ])
        w.writerow(row)

def main():
    ap = argparse.ArgumentParser(description="Fetch FEMA Disaster Declarations into enriched NDJSON.")
    ...
    ap.add_argument("--pre-days", type=int, default=PRE_DAYS_DEFAULT, help="Days before incidentBeginDate")
    ap.add_argument("--post-days", type=int, default=POST_DAYS_DEFAULT, help="Days after incidentEndDate")
    ...
    while True:
        ...
        for d in rows:
            ...
            # Build the base date objects
            begin_dt = parse_date(incident_begin) or parse_date(decl_dt)
            end_dt   = parse_date(incident_end)   or parse_date(decl_dt)

            if not begin_dt or not end_dt:
                skipped_no_dates += 1
                write_log_row(log_path, [
                    decl_dt, d.get("state",""), "", "",
                    d.get("incidentType",""), "", d.get("disasterNumber",""),
                    "", "", "no_dates"
                ])
                continue

            # Normalize to UTC and compute window
            if begin_dt.tzinfo is None:
                begin_dt = begin_dt.replace(tzinfo=timezone.utc)
            if end_dt.tzinfo is None:
                end_dt = end_dt.replace(tzinfo=timezone.utc)

            window_start = (begin_dt - timedelta(days=args.pre_days)).date().isoformat()
            window_end   = (end_dt   + timedelta(days=args.post_days)).date().isoformat()
            ...
            if not (fips2.isdigit() and state_num.isdigit()):
                ...
                write_log_row(log_path, [
                    decl_dt, d.get("state",""), "", "",
                    incident_type, normalized or incident_type.lower(), d.get("disasterNumber",""),
                    window_start, window_end, "bad_fips"
                ])
                continue
            ...
            if fips5 not in fips_map:
                skipped_no_map += 1
                write_log_row(log_path, [
                    decl_dt, d.get("state",""), fips5, "",
                    incident_type, normalized or incident_type.lower(), d.get("disasterNumber",""),
                    window_start, window_end, "fips_not_in_county_map"
                ])
                continue
            ...
            decl_title = d.get("declarationTitle") or ""
            designated = d.get("designatedArea") or ""

            # Human-friendly label for GDELT queries / plots
            label = f"{county_clean}, {state_name} — {incident_type or 'Disaster'}"

            rec = {
                "state": state_name,
                "county": county_clean,
                "county_fips": fips5,
                "date": decl_dt,  # declaration date (YYYY-MM-DD)
                "incident_begin": norm_date(incident_begin) if incident_begin else None,
                "incident_end": norm_date(incident_end) if incident_end else None,
                "window_start": window_start,
                "window_end": window_end,
                "disaster": normalized or incident_type.lower(),
                "fema_incident_type": incident_type,
                "fema_declaration_type": decl_type,
                "fema_declaration_title": decl_title,
                "fema_designated_area": designated,
            ...
            write_log_row(
                log_path,
                [decl_dt, state_name, fips5, county_clean, incident_type, rec["disaster"], dis_num, window_start, window_end, ""]
            )
    ...
    print(f"Skipped {skipped_no_dates} with no usable dates, {skipped_bad_fips} with bad FIPS, {skipped_no_map} not in county map.")
